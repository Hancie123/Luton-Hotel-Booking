...
public class EmployeesDetails {
	...
	public EmployeesDetails() {
		frame=new JFrame("All EMPLOYEES DETAILS");
		...
		customer=new JLabel("EMPLOYEES DETAILS", SwingConstants.CENTER);
		customer.setBounds(500,20,300,35);
		customer.setForeground(Color.white);
		customer.setFont(new Font("Verdana", Font.BOLD, 18));
		north.add(customer);
		
		JLabel searchlbl=new JLabel("Search Employees: ");
		...
		searchField.setBounds(210,25,200,30);
	...
	ex.printStackTrace();
		...
		JDBCEmployees jdbc2 = new JDBCEmployees();
		ArrayList select = jdbc2.select_all();
		if(select.size()>0) {
			for(int i=0; i<select.size(); i++) {
				EmployeesLibs p = (EmployeesLibs) select.get(i);
				Object []tmp= {p.getID(), 
								p.getName(), 
								p.getAge(),
								p.getMobile(),	
